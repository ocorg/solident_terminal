...
const FROM     = `"Solident" <${process.env.GMAIL_USER}>`
          ...
          <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
            <tr>
              <td style="background:linear-gradient(135deg,#1E5F7A,#2a7a9a);padding:32px 40px;text-align:center;">
                ...
                <h1 style="margin:0;color:#ffffff;font-size:22px;font-weight:700;">Solident</h1>
                <p style="margin:4px 0 0;color:rgba(255,255,255,0.7);font-size:13px;">Bridge de Solidarite des Medecins Dentistes</p>
              </td>
            </tr>
            <tr>
...
export async function sendDigestEmail(to: string, fullName: string, actions: any[]) {
  ...
  const subject = count === 1 ? `Solident — 1 nouvelle notification` : `Solident — ${count} nouvelles notifications`
  const items = actions.map(a => {
    const meta = ACTION_LABELS[a.action_type] || { icon: '🔔', label: a.action_type, color: '#f8fafc' }
    return card(meta.color, meta.icon, meta.label, a.payload?.title || a.payload?.name || a.payload?.detail || '')
  }).join('')
  await sendEmail(to, subject, template(
    `Bonjour ${fullName}, vous avez ${count} notification${count > 1 ? 's' : ''}`,
    `${para('Voici un résumé de vos dernières activités sur Solident :')}
    <div style="margin:20px 0;">${items}</div>
    ${para('Connectez-vous pour voir les détails.')}
    <div style="text-align:center;margin:24px 0;">${btn('Accéder à la plateforme', BASE_URL)}</div>`
  ))
}

export async function emailInvite(to: string, fullName: string, activationLink: string) {
  await sendEmail(to, 'Bienvenue sur Solident — Activez votre compte', template(
    `Bienvenue, ${fullName} ! 🎉`,
    `${para("Vous avez été invité(e) à rejoindre la plateforme <strong>Solident</strong>.")}
    ...
    ${para("Cliquez sur le bouton ci-dessous pour définir votre mot de passe et accéder à la plateforme.")}
    <div style="text-align:center;margin:24px 0;">${btn('✨ Activer mon compte', activationLink)}</div>
    <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:10px;padding:12px 16px;margin-top:16px;">
      <p style="margin:0;color:#92400e;font-size:12px;">⏰ <strong>Ce lien expire dans 24 heures.</strong></p>
    </div>`
  ))
}

export async function emailPasswordReset(to: string, resetLink: string) {
  await sendEmail(to, 'Réinitialisation de votre mot de passe Solident', template(
    '🔐 Réinitialisation de mot de passe',
    `${para("Vous avez demandé la réinitialisation de votre mot de passe <strong>Solident</strong>.")}
    ${para("Ce lien est valable <strong>1 heure</strong>.")}
    <div style="text-align:center;margin:28px 0;">${btn('🔑 Réinitialiser mon mot de passe', resetLink)}</div>
    <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:10px;padding:14px 18px;margin-top:16px;">
      <p style="margin:0 0 6px;color:#991b1b;font-size:13px;font-weight:700;">⚠️ Sécurité</p>
      <p style="margin:0;color:#b91c1c;font-size:12px;">Si vous n'avez pas demandé cette réinitialisation, ignorez cet email.</p>
    </div>`
  ))
}