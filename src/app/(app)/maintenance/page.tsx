...
function queueStatusColor(s: string) {
  ...
  if (s === 'warning')  return 'text-[#F0A500] bg-[#F0A500]/10 border-[#F0A500]/20'
  return 'text-green-400 bg-green-500/10 border-green-500/20'
}

function queueStatusLabel(s: string) {
  if (s === 'critical') return '🔴 Bloquée'
...
export default function MaintenancePage() {
    ...
    async function init() {
      ...
      if (memberData) {
        ...
        if (memberData.length > 0) {
          setNotifUserId(user.id) // default to self
  ...
  async function checkSmtp() {
    ...
    setSmtpOp({
      ...
      message: d.success ? `OK — ${d.account}` : (d.error || 'Échec'),
    })
  }
  ...
  async function triggerCron() {
    ...
    const d = await res.json()
    if (d.success) {
      setCronOp({ status: 'success', message: `Cron exécuté — ${d.result?.sent ?? 0} email(s) envoyé(s)` })
      await loadAll()
    } else {
      setCronOp({ status: 'error', message: d.error || `Status ${d.status}` })
    }
  }
  ...
  if (!profile?.is_admin && !profile?.is_backoffice) return (
    <div className="flex items-center justify-center h-64">
      <p className="text-white/40 text-sm">Accès restreint au Backoffice</p>
    </div>
  )
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      {/* ── Header Banner ── */}
      <div className="relative overflow-hidden bg-gradient-to-br from-[#0f1f2e] via-[#0a1828] to-[#050c18] border border-white/10 rounded-2xl p-6">
        <div className="absolute top-0 right-0 w-64 h-64 bg-[#1E5F7A]/10 rounded-full -translate-y-1/3 translate-x-1/4 blur-2xl pointer-events-none" />
        <div className="relative flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-[#1E5F7A]/20 border border-[#1E5F7A]/30 flex items-center justify-center text-xl flex-shrink-0">
            🛠️
          </div>
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              <h1 className="text-white font-bold text-lg">Maintenance</h1>
              <span className="text-[10px] bg-[#F0A500]/20 text-[#F0A500] border border-[#F0A500]/30 px-2 py-0.5 rounded-full font-semibold">
                BACKOFFICE
              </span>
            </div>
            <p className="text-white/40 text-xs">Outils de diagnostic et de contrôle système</p>
          </div>
          {health && (
            <div className="ml-auto text-right flex-shrink-0">
              <p className="text-white/30 text-[10px]">Dernière sync</p>
              <p className="text-white/50 text-xs">{timeAgo(health.timestamp)}</p>
            </div>
          )}
        </div>
      </div>
        ...
        {[
          {
            ...
            value: health?.health.database === 'ok' ? 'En ligne' : 'Erreur',
            icon:  '🗄️',
            ok:    health?.health.database === 'ok',
          },
          {
            label: 'Utilisateurs',
            value: `${health?.health.total_users ?? '—'} membres`,
            icon:  '👥',
            ok:    true,
          },
          {
            label: 'Queue email',
            value: `${health?.health.email_queue.count ?? '—'} en attente`,
            icon:  '📧',
            ok:    queueStatus === 'healthy',
          },
          {
            label: 'Dernière notif',
            value: health?.health.last_notification ? timeAgo(health.health.last_notification) : 'Aucune',
            icon:  '🔔',
            ok:    !!health?.health.last_notification,
          },
        ].map(s => (
          <div key={s.label} className={`border rounded-2xl p-4 ${s.ok ? 'bg-white/5 border-white/10' : 'bg-red-500/5 border-red-500/20'}`}>
            <div className="flex items-center gap-2 mb-1">
              <span>{s.icon}</span>
              <span className={`text-[10px] font-semibold ${s.ok ? 'text-green-400' : 'text-red-400'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card title="Mode Maintenance" icon="🔒">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className={`ml-4 text-xs font-bold px-3 py-1.5 rounded-full border flex-shrink-0 ${
              health?.maintenance_mode
                ? 'bg-[#F0A500]/10 border-[#F0A500]/20 text-[#F0A500]'
                : 'bg-green-500/10 border-green-500/20 text-green-400'
            }`}>
          ...
          <button
            ...
            className={`w-full py-2.5 rounded-xl text-sm font-semibold transition border ${
              health?.maintenance_mode
                ...
                : 'bg-[#F0A500]/10 border-[#F0A500]/20 text-[#F0A500] hover:bg-[#F0A500]/20'
            } disabled:opacity-50`}
          >
        ...
        <Card title="Vérification SMTP" icon="📨">
          ...
          <div className="flex items-center gap-2 mb-3 text-xs text-white/30 bg-white/5 rounded-lg px-3 py-2 border border-white/5">
            <span>📬</span>
            <span className="font-mono truncate">{process.env.NEXT_PUBLIC_APP_URL ? 'smtp.gmail.com:465' : 'smtp.gmail.com:465'}</span>
          </div>
          <button
            onClick={checkSmtp}
            disabled={smtpOp.status === 'loading'}
            className="w-full py-2.5 bg-[#1E5F7A]/20 border border-[#1E5F7A]/30 text-[#5bbcde] rounded-xl text-sm font-semibold hover:bg-[#1E5F7A]/30 transition disabled:opacity-50"
          >
        ...
        <Card title="Queue Email" icon="📧">
          <div className={`flex items-center justify-between mb-4 text-xs px-3 py-2 rounded-lg border ${queueStatusColor(queueStatus)}`}>
            <span className="font-semibold">{queueStatusLabel(queueStatus)}</span>
          ...
          {queue.length > 0 ? (
            <div className="space-y-1.5 max-h-36 overflow-y-auto mb-4">
              {queue.map(item => (
                <div key={item.id} className="flex items-center gap-2 text-xs bg-white/5 rounded-lg px-3 py-2 border border-white/5">
                  <span className="text-white/30">📧</span>
                  <span className="text-white/60 truncate flex-1">{item.recipient_email}</span>
                  <span className="text-white/30 flex-shrink-0">{item.action_type}</span>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-white/30 text-xs text-center py-3 mb-4">Queue vide ✅</p>
          )}
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className="py-2.5 bg-[#1E5F7A]/20 border border-[#1E5F7A]/30 text-[#5bbcde] rounded-xl text-xs font-semibold hover:bg-[#1E5F7A]/30 transition disabled:opacity-50"
            >
            ...
            <button
              ...
              className="py-2.5 bg-[#F0A500]/10 border border-[#F0A500]/20 text-[#F0A500] rounded-xl text-xs font-semibold hover:bg-[#F0A500]/20 transition disabled:opacity-50"
            >
        ...
        <Card title="Test Notification In-app" icon="🔔">
            ...
            {(['user', 'all'] as const).map(t => (
              <button key={t} onClick={() => setNotifTarget(t)}
                className={`flex-1 py-2 rounded-lg text-xs font-semibold border transition ${
                  notifTarget === t
                    ? 'bg-[#1E5F7A]/30 border-[#1E5F7A]/50 text-[#5bbcde]'
          ...
          {notifTarget === 'user' && (
            <select
              ...
              className="w-full mb-3 bg-white/5 border border-white/10 text-white text-xs rounded-lg px-3 py-2 focus:outline-none focus:border-[#1E5F7A]/50"
            >
              {members.map(m => (
                <option key={m.id} value={m.id} className="bg-[#0e1628]">
          ...
          <input
            ...
            className="w-full mb-3 bg-white/5 border border-white/10 text-white text-xs rounded-lg px-3 py-2 placeholder-white/20 focus:outline-none focus:border-[#1E5F7A]/50"
          />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col justify-end">
              <div className="bg-white/5 border border-white/5 rounded-xl p-3 mb-3">
                <p className="text-white/30 text-[10px] mb-1">Email qui sera envoyé :</p>
                <p className="text-white/60 text-xs font-mono">action_type: task_assigned</p>
                <p className="text-white/40 text-[10px] mt-0.5">Template digest Solident</p>
              </div>
              <button
      ...
      {/* ── Footer Note ── */}
      <p className="text-white/20 text-[11px] text-center pb-2">
