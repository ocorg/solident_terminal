...
sequenceDiagram
  ...
  participant T as Treasurer (/admin)
  participant P as Pusher
  D->>W: Sees campaign, copies RIB, makes transfer
  D->>W: Submits "J'ai fait un don"
  W->>DB: Insert donation (pending)
  W-->>D: Toast + thank-you email
  W->>T: Alert email + admin badge
  T->>W: Confirms donation
  W->>DB: Status confirmed, recalc total (1 transaction)
  W->>P: campaign-{id} total-updated
  P-->>D: Bar animates to new %
```
